...
const makeIconHtml = (size) => `<!DOCTYPE html>
<html><head>
<link href="https://fonts.googleapis.com/css2?family=Righteous&display=swap" rel="stylesheet">
<style>
*{margin:0;padding:0;box-sizing:border-box}
body{width:${size}px;height:${size}px;overflow:hidden;background:#111}
.record{width:${size}px;height:${size}px;border-radius:50%;position:absolute;inset:0}
...
const makeInAppHtml = (size) => `<!DOCTYPE html>
<html><head>
<link href="https://fonts.googleapis.com/css2?family=Righteous&display=swap" rel="stylesheet">
<style>
*{margin:0;padding:0;box-sizing:border-box}
...
(async () => {
  ...
  // App drawer icons — dark opaque background, record fills edge to edge
  for (const [density, size] of Object.entries(sizes)) {
    const page = await browser.newPage();
    await page.setViewport({ width: size, height: size, deviceScaleFactor: 1 });
    await page.setContent(makeIconHtml(size), { waitUntil: 'networkidle0' });
    ...
    // Full page screenshot — includes the dark background
    ...
    console.log('App icon ' + density + ': ' + size + 'px (opaque bg)');
    await page.close();
  }

  // In-app logo — transparent background for spinning over dark UI
  const logoPage = await browser.newPage();
  await logoPage.setViewport({ width: 512, height: 512, deviceScaleFactor: 1 });
  await logoPage.setContent(makeInAppHtml(512), { waitUntil: 'networkidle0' });
  ...
  const el = await logoPage.$('#target');
  await el.screenshot({ path: path.join(drawableDir, 'vinyl_record.png'), type: 'png', omitBackground: true });
  console.log('In-app logo: 512px (transparent bg)');
