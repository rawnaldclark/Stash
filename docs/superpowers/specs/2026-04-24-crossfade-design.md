...
**`ForwardingPlayer` footgun — must override everything that depends on "active".** `ForwardingPlayer` is built on a fixed `wrappedPlayer` reference set at construction time; all non-overridden methods route there. Because the wrapper *rotates* the active player between A and B on every fade-swap, every getter/setter that should reflect the *current* active player must be explicitly overridden to route through `activeRef.get()`. The simplest implementation pattern: override **every method** the wrapper cares about (the table in section 3) to consult `activeRef`/`nextRef` based on the current state, and let `ForwardingPlayer`'s default delegation handle only methods that are state-agnostic (e.g., `getApplicationLooper`, `getDeviceInfo`, etc.). Don't assume unoverridden methods "just work" after a swap.
...
**Both players share construction:** the same `LoadControl`, `AudioAttributes`, `setHandleAudioBecomingNoisy(true)`, `setWakeMode(C.WAKE_MODE_LOCAL)`, and **the same `audioSessionId`** as the existing single `ExoPlayer`. The `audioSessionId` MUST be shared so the equalizer (initialized once via `equalizerManager.initialize(sharedSessionId)`) applies to both players' audio. Set `playerB.audioSessionId = sharedSessionId` before `equalizerManager.initialize` runs. **One difference:** only `playerA` is built with `handleAudioFocus = true`. `playerB` inherits focus from the same audio session and never makes its own focus request, avoiding focus-thrash during overlap.
...
- **`seekTo(positionMs)` during a crossfade.** Conceptually: "user wants to jump somewhere — abort the fade, land cleanly on the incoming track." The wrapper applies the seek to `nextPlayer` (the incoming track), then snaps `activePlayer.volume = 0` and `nextPlayer.volume = 1.0` to abort the fade. Rationale: by the time a crossfade is in flight, the wrapper's `currentMediaItem` already reports the incoming track (per section 3's forwarding rules), so any external seek is a seek on the incoming track. Treating the incoming track as the seek target is consistent with the "what is currently playing" view that controllers see
